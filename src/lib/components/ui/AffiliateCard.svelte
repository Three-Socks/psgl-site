...
>
    ...
    {#if item.code}
        ...
        >
            ...
            {:else}
                {item.discount}: <span class="text-psgl-blue-2">{item.code}</span>
                <Copy class="h-4 w-4 text-white/50" />
            {/if}
        </button>
    {/if}

    <div class="text-psgl-blue-2 mt-auto flex items-center gap-2 text-xs font-bold uppercase tracking-[0.2em] transition-all duration-300 group-hover:text-white">
